...
export const FAQ: Record<Lang, [string, string][]> = {
  et: [
    ...
    ['Kas siin saab pidu või sündmust pidada?',
     'Jah, kokkuleppel. Pidulik pikk laud kaetakse ette tellides kahele kuni kümnele, suvel väliköögis kuni kahekümnele. Tingimused ja kord lepitakse peremehega enne kokku – kirjuta info@oanduaia.ee.'],
    ['Kas lisavoodeid saab?',
     'Jah. Igasse majja saab lisavoodi. Lehel olevad arvud – Saunamaja viiele, Metsamaja neljale, Tiigimaja kahele – on see, mitu inimest maja tavapäraselt magab; lisavoodi tuleb broneerides eraldi kinnitada. Kirjuta info@oanduaia.ee.'],
  ...
  en: [
    ...
    ['Can we hold a celebration or an event here?',
     'Yes, by arrangement. The festive long table is laid to order for two to ten people, and in summer for up to twenty in the outdoor kitchen. The terms and the house rules are agreed with the owner beforehand — write to info@oanduaia.ee.'],
    ['Can you add extra beds?',
     'Yes. An extra bed can be added in any of the three houses. The numbers on this site — five in the Sauna House, four in the Forest House, two in the Pond House — are what each house sleeps as standard; an extra bed is confirmed with us when you book. Write to info@oanduaia.ee.'],
    ['Are pets allowed?',
  ...
  ru: [
    ...
    ['Можно ли провести здесь праздник или мероприятие?',
     'Да, по договорённости. Праздничный длинный стол накрывают по предварительному заказу на двух-десять человек, летом в летней кухне — до двадцати. Условия и правила согласуются с хозяином заранее — напишите на info@oanduaia.ee.'],
    ['Можно ли добавить кровати?',
     'Да. Дополнительную кровать можно поставить в любом из трёх домов. Числа на сайте — пять в Банном доме, четыре в Лесном, двое в Прудовом — это стандартная вместимость; дополнительную кровать подтверждаем при бронировании. Напишите на info@oanduaia.ee.'],
